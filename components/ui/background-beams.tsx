...
export const BackgroundBeams = React.memo(
  ({
    ...
    useEffect(() => {
      ...
      if (!repeating) {
        setIsVisible(true); // Show the path only if not repeating
      } else {
        setIsVisible(true); // Show the path if repeating
      }
    }, [repeating]);
        ...
        >
          {isVisible && (
            <motion.path
              ...
              strokeWidth="10"
              animate={{
                pathLength: repeating ? [0, 1, 1, 0] : [0, 1, 1, 1], // Draw → Hold → Instantly Reset
              }}
              transition={{
                duration: 15, // 5s draw time
                times: [0, 0.3, 1, 1], // Instant reset between 0.8-0.81
          ...
          {repeating && (
            <motion.path
              ...
              strokeWidth="20"
              animate={{
                pathLength: [0, 0, 1, 1, 0], // Wait → Draw → Hold → Instantly Reset
              }}
              transition={{
                duration: 15, // 5s covering time
                times: [0, 0.7, 1, 1, 1], // Instant reset between 0.8-0.81
                repeat: Infinity,
                ease: "linear",
                repeatDelay: 2,
              }}
